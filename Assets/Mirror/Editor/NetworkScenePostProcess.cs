        ...
        public static void OnPostProcessScene()
        {
            ...
            foreach (NetworkIdentity identity in identities)
            {
                ...
                if (!identity.isClient && !identity.isServer)
                {
                    ...
                    else Debug.LogError("Scene " + identity.gameObject.scene.path + " needs to be opened and resaved, because the scene object " + identity.name + " has no valid sceneId yet.", identity.gameObject);
                }
            }
        }
